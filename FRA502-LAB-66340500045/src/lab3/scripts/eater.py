...
from std_msgs.msg import Bool,Int16,Float64
...
class EaterNode(Node):
    ...
    def set_pizza_callback(self,request:SetMaxPizza.Request ,response:SetMaxPizza.Response):
        self.set_pizza = request.max_pizza.data
    ...
    def control(self):
        msg = Bool()
        if self.c == self.set_pizza and self.set_pizza != 0:
                msg.data = True
                self.de = False
        if self.count > 0 :
            if (self.set_pizza - self.c > 0) :
                msg.data = False
            ...
            self.control_robot[3] = math.sqrt((self.control_robot[0]**2)+(self.control_robot[1]**2)) -1#d
    ...
    def timer_callback(self):
            if self.count > 0  and (self.kp != 0) and (self.kp_a != 0) and self.set_pizza > 0 :
                self.control()
            else :
                self.cmdvel(0.0,0.0)
            if self.de :
